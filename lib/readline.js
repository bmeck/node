...
const ucsCharPattern = '(?:[\uD800-\uDBFF][\uDC00-\uDFFF]|.)';
const ucsIncompleteCharHigh = /^[\uD800-\uDBFF]$/;
const ucsIncompleteCharLow = /^[\uDC00-\uDFFF]$/;
const firstCharRE = new RegExp('^'+ucsCharPattern);
const lastCharRE = new RegExp(ucsCharPattern+'$');
...
Interface.prototype._deleteLeft = function() {
  if (this.cursor > 0 && this.line.length > 0) {
    lastCharRE.lastIndex = 0;
    let l = (this.line.slice(0, this.cursor));
    let last_ch = lastCharRE.exec(this.line.slice(0, this.cursor));
    if (!last_ch) return;
    // grab full character if cursor is in between high and low surrogate
    if (lastCharRE.lastIndex !== this.line.length && ucsIncompleteCharHigh.test(last_ch)) {
      firstCharRE.lastIndex = lastCharRE.lastIndex - last_ch.length;
      last_ch = firstCharRE.exec(this.line);
      this.cursor = firstCharRE.lastIndex;
    }
    let left = this.line.slice(0, this.cursor - last_ch[0].length);
    let right = this.line.slice(this.cursor, this.line.length);
...
Interface.prototype._deleteRight = function() {
  if (this.line.length > 0  && this.cursor !== this.line.length) {
    firstCharRE.lastIndex = this.cursor;
    let first_ch = firstCharRE.exec(this.line);
    if (!first_ch) return;
    // grab full character if cursor is in between high and low surrogate
    if (firstCharRE.firstIndex !== this.line.length && ucsIncompleteCharLow.test(first_ch)) {
      lastCharRE.lastIndex = firstCharRE.lastIndex;
      first_ch = lastCharRE.exec(this.line);
    }
    let left = this.line.slice(0, this.cursor - first_ch[0].length);
    let right = this.line.slice(this.cursor + first_ch[0].length, this.line.length);
    this.line = left + right;
    this.cursor = left.length;
    this._refreshLine();
  }
...
Interface.prototype._moveCursor = function(dx) {
  ...
  // bounds check
  if (this.cursor + dx < 0) this.cursor = 0;
  else if (this.cursor + dx > this.line.length) this.cursor = this.line.length;
  else if (dx < 0) {
    let fifo = [];
    let index = 0;
    for (let k of this.line) {
      if (fifo.length === -dx) {
        fifo.shift();
      }
      fifo.push(index);
