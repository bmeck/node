...
function processTopLevelAwait(src, env) {
  ...
  const ctxId = generateHygenicIdentifier();
  const ret = wrappedArray.map((item) => {
    ...
    return item;
      ...
      .replace(WRAPPED_PREFIX,
               `(async (${ctxId}) => { with (${ctxId}.env) {`)
      .slice(0, -WRAPPED_SUFFIX.length) +
          `; return ${ctxId}.completionValue; }` +
          WRAPPED_SUFFIX.replace('()', '');
  return ret;
}
