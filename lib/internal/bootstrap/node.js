...
function evalScript(name, body) {
  ...
  const [ state, value ] = getPromiseDetails(result);
  if (state === kRejected) throw value;
  else if (process._print_eval) {
    if (state === kFulfilled) console.log(value);
    else result.then((value) => {
      console.log(value);
    });
  }
