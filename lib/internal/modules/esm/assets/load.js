...
/**
 * Lazily load to avoid snapshot issues
 */
  ...
  const result = new Promise((fulfill, reject) => {
    ...
    .on('response', (res) => {
      ...
      if (res.statusCode >= 300 && res.statusCode <= 303) {
        if (res.headers.location) {
          ...
          if (location.protocol !== 'http:' && location.protocol !== 'https:') {
            reject(new Error('cannot redirect to non-network location'));
            return;
          }
      ...
      if (res.statusCode > 303 && res.statusCode < 200) {
        dispose();
        reject(
          new Error('Http response returned status code: ' + res.statusCode));
        return;
      }
      const { headers } = res;
      ...
      if (!contentType) {
        dispose();
        reject(new Error('content-type is required'));
        return;
      }
      /**
       * @type {CacheEntry}
...
function fetch(parsed) {
  ...
  if (parsed.protocol === 'http:') {
    return isLocalAddress(parsed.hostname).then((is) => {
      if (is !== true) {
        throw new Error(`Cannot load non-local address ${href} from network using http: protocol, use https: instead`);
      }
