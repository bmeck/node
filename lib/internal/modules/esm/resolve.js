...
async function defaultResolve(specifier, context = {}, defaultResolveUnused) {
  ...
  if (parentURL && parsed) {
    ...
    if (
      ...
      // data: and blob: disallowed due to allowing file: access via indirection
      if (
        parsed.protocol !== 'https:' &&
        parsed.protocol !== 'http:'
      ) {
        throw new Error(`Cannot import local location ${specifier} from remote location ${parentURL}, load a it via a remote location.`);
      }
    }
  }
...
if (policy) {
  ...
  module.exports.defaultResolve = async function defaultResolve(...args) {
    const ret = await $defaultResolve(...args);
    if (policy.manifest.mightAllow(ret.url)) {
      return ret;
    }
    throw new Error(`Unable to resolve ${ret.url}, no entry in manifest`);
  };
}
