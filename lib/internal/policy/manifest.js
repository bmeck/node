...
class Manifest {
  ...
  constructor(obj, manifestURL) {
    ...
    for (let i = 0; i < jsonResourcesEntries.length; i++) {
      ...
      const maxSize = resourceDescriptor.maxSize;
      console.log(originalHREF, resourceDescriptor)
      ...
      if (typeof maxSize === 'number') {
        this.#resourceMaxSizes.set(resourceHREF, maxSize);
    ...
    for (let i = 0; i < jsonScopesEntries.length; i++) {
      ...
      const maxSize = scopeDescriptor.maxSize;
      console.log(originalHREF, scopeDescriptor, maxSize)
      if (typeof integrity !== 'undefined') {
      ...
      if (typeof maxSize === 'number') {
        this.#scopeMaxSizes.set(resourceHREF, maxSize);
      } else if (maxSize !== undefined) {
        throw new ERR_MANIFEST_INVALID_RESOURCE_FIELD(
          resourceHREF,
          'maxSize');
  ...
  assertMaxSize(url, size) {
    const href = `${url}`;
    debug('Checking max size of %s', href);
    const maxSizes = this.#resourceMaxSizes;
    if (maxSizes.has(href)) {
      const maxSize = maxSizes.get(href);
      if (maxSize === -1) return true;
      if (size > maxSize) {
        const error = new ERR_MANIFEST_ASSERT_INTEGRITY(url+'?maxSize', new Map());
        this.#reaction(error);
      }
    }
    let scope = this.#findScopeHREF(href);
    console.log(this.#scopeMaxSizes, scope, typeof scope)
    while (scope !== null) {
      if (this.#scopeMaxSizes.has(scope)) {
        const maxSize = this.#scopeMaxSizes.get(scope);
        console.log(this.#scopeMaxSizes, scope, typeof scope, maxSize, size)
        if (maxSize === -1) {
          return true;
        } else {
          if (size > maxSize) {
    ...
    const error = new ERR_MANIFEST_ASSERT_INTEGRITY(url+'?maxSize2 s', new Map());
    this.#reaction(error);
  }
