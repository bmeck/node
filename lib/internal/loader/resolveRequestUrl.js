...
class BlobModuleRequest extends ModuleRequest {
  constructor(url) {
    super(url);
    const blob = internalURLModule.blobStore.get(url.href);
    const {bytes, type} = internalFS.fileStore.get(this);
    if (type === 'text/javascript' || type === 'application/javascript') {
      // ESM
      this.wrap = new ModuleWrap(internalCJSModule.stripShebang(source),
                            `${this.url}`);
    }
    throw new errors.Error('ERR_INVALID_TYPE', type, ['text/javascript']);
  }
  async createModule() {
    return this.wrap;
  }
...
const resolveRequestUrl = (baseURLOrString, specifier) => {
  ...
  if (url.protocol === 'blob:') {
    if (internalURLModule.blobStore.has(url.href)) {
      throw new errors.Error('ERR_MISSING_MODULE', url.href);
    }
    return new BlobModuleRequest(url);
  }
  if (url.protocol !== 'file:') {
    throw new errors.Error('ERR_INVALID_PROTOCOL', url.protocol, ['file:', 'blob:']);
  }
