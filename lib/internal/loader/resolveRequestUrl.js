...
class CJSModuleRequest extends ModuleRequest {
  async createModule() {
    ...
    let exportNames = undefined;
    if (extname(pathname) === '.js') {
      const content = await asyncReadFile(pathname, 'utf8');
      const src = internalCJSModule.stripBOM(content);
      exportNames = require('internal/module').getExportPrologueNames(src);
    }
    const ctx = createDynamicModule(exportNames ? [...exportNames] : ['default'], this.url, (reflect) => {
      debug(`Loading CJSModule ${this.url.pathname}`);
      const CJSModule = require('module');
      CJSModule._load(pathname);
    });
    this.finish = (module) => {
      if (!exportNames) ctx.reflect.exports.default.set(module.exports);
    };
    return ctx.module;
  }
}
