...
const createExportInvariantProxy = function ({
  ...
  for (const key of REFLECT.ownKeys(target)) {
    ...
    if (bindings.has(key)) {
      if (!desc) {
        throw TYPE_ERROR(`cannot setup invariants on missing property ${key}`)
      }
      else {
        if (!hasOwnProperty(desc, 'value')) {
          throw TYPE_ERROR(`cannot setup invariants on getter/setter for ${key}.`);
        }
      }
    }
  }
  const handlers = {
    ...
    get(target, property, receiver) {
      ...
      if (desc && Object.hasOwnProperty.call(desc, 'get')) {
        console.log('CALLING w/ receiver', property, this)
        return REFLECT.apply(desc.get, receiver, []);
      }
      console.log('CALLING w/ target', property)
