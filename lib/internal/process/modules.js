  ...
  const loaderPromise = (async () => {
    ...
    if (userLoaders) {
      ...
      for (var i = 0; i < userLoaders.length; i++) {
        ...
        let cachedResolve = resolve;
        let cachedDynamicInstantiate = dynamicInstantiate;
        let next = factory({
          __proto__: null,
          ...
          dynamicInstantiate: Object.setPrototypeOf(async url => {
                const ret = await dynamicInstantiate(url);
                return {
                  __proto__: null,
                  exports: ret.exports,
                  execute: ret.execute,
        ...
        if (resolveDesc !== undefined) {
          ...
          if (typeof resolve !== 'function') {
            throw new errors.TypeError('ERR_LOADER_HOOK_BAD_TYPE', 'resolve', 'function');
          }
        }
        const dynamicInstantiateDesc = getOwnPropertyDescriptor(next, 'dynamicInstantiate');
        if (dynamicInstantiateDesc !== undefined) {
          ...
          if (typeof dynamicInstantiate !== 'function') {
            throw new errors.TypeError('ERR_LOADER_HOOK_BAD_TYPE', 'dynamicInstantiate', 'function');
          }
        }
      }
      ESMLoader.hook({
